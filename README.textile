...
This code is in development, and is very simple. You can only read from Spotlight, currently I don't have any plans to write back to Spotlight. Attempting to write to the attributes hash will raise an error. This is intentional, so if I do find myself trying to do this, I should implement the functionality.
...
Most of the C code was extracted from http://github.com/xli/spotlight
Unfortunately for me xli's gem doesn't compile in ruby 1.9.2
One day i'll have some time to really figure out why

My gem is very very simple, if you need to write back to spotlight then have a look at xli's gem
...
Copyright (c) 2010 Dougal MacPherson. See LICENSE for details.
